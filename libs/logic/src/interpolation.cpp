                                ...
                                int accuracy)
{
    ...
    for (int i = 0; i <= accuracy; i++) {
        ...
        double x = h00 * p1.x_int() + h10 * force1.x_int() + h01 * p2.x_int() +
                   h11 * force2.x_int();
        double y = h00 * p1.y_int() + h10 * force1.y_int() + h01 * p2.y_int() +
                   h11 * force2.y_int();
                               ...
                               int accuracy)
{
    return draw_hermite(p1, {3 * (p2.x - p1.x), 3 * (p2.y - p1.y), 0}, p4,
                        {3 * (p4.x - p3.x), 3 * (p4.y - p3.y), 0}, accuracy);
}
                                         ...
                                         Point P_ip2, int accuracy)
{
    ...
    for (int k = 0; k <= accuracy; k++) {
        ...
        double x = b0 * P_im1.x + b1 * P_i.x + b2 * P_ip1.x + b3 * P_ip2.x;
        double y = b0 * P_im1.y + b1 * P_i.y + b2 * P_ip1.y + b3 * P_ip2.y;
