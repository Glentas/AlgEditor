...
std::vector<Point> draw_wu(Point p1, Point p2)
{
	...
	for (int x = x1; x <= x2; x++) {
		...
		if (steep) {
			points.push_back({ y_floor, static_cast<double>(x),
					   static_cast<uint8_t>(255 * frac) });
			points.push_back(
				{ y_floor + 1.0, static_cast<double>(x),
				  static_cast<uint8_t>(255 * (1.0 - frac)) });
		} else {
			points.push_back({ static_cast<double>(x), y_floor,
					   static_cast<uint8_t>(255 * frac) });
			points.push_back(
				{ static_cast<double>(x), y_floor + 1.0,
				  static_cast<uint8_t>(255 * (1.0 - frac)) });
		}
		y += gradient;
	}
