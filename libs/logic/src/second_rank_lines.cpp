...
std::vector<Point> draw_circle(Point center, int radius)
{
    ...
    for (int i = 0; i < sz; i++) {
        points.push_back({static_cast<double>(points[i].y_int()),
                          static_cast<double>(-points[i].x_int()), 0});
    }
    sz = points.size();
    for (int i = 0; i < sz; i++) {
...
std::vector<Point> draw_ellipse(Point center, int a, int b)
{
    ...
    points.push_back({0.0, static_cast<double>(b), 0.0, 0});
    ...
    while (y > 0) {
        if (err > 0) {
            ...
            if (lambda <= 0) {
                ...
                y -= 1;
                err =
                    err + pow(b, 2) * (2.0 * x + 1) + pow(a, 2) * (1 - 2.0 * y);
            } else {
                y -= 1;
...
std::vector<Point> draw_hyperbola(Point center, int a, int b)
{
    ...
    auto add_symmetric = [&](int dx, int dy) {
        points.push_back(
            {static_cast<double>(c_x + dx), static_cast<double>(c_y + dy), 0});
        if (dy != 0) {
    ...
    for (int y = 0; y <= MAX_Y_OFFSET; ++y) {
        double x_exact = a * sqrt(1.0 + (y * y) / static_cast<double>(b * b));
